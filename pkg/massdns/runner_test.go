...
func TestCreateMassdnsArgs_DefaultRateLimit(t *testing.T) {
	...
	assert.ElementsMatch(t, []string{"-q", "-r", "resolvers.txt", "-o", "S", "-t", "A", "--retry", "REFUSED", "--retry", "SERVFAIL", "-w", "output.txt"}, gotArgs)
}

func TestCreateMassdnsArgs_CustomRateLimit(t *testing.T) {
	runner := defaultRunner{}
	gotArgs := runner.createMassdnsArgs("output.txt", "resolvers.txt", 100)
	assert.ElementsMatch(t, []string{"-q", "-r", "resolvers.txt", "-o", "S", "-t", "A", "--retry", "REFUSED", "--retry", "SERVFAIL", "-w", "output.txt", "-s", "100"}, gotArgs)
}
